...
- Extract table content with row/column structure
...
Classes:
    ...
    TableData: Represents a table with cells organized by row/column
...
TableDict = Dict[str, Any]  # Table structure with cells
InventoryData = Dict[
    str, Dict[str, Union["ShapeData", "TableData"]]
]  # Dict of slide_id -> {shape_id/table_id -> ShapeData/TableData}
InventoryDict = Dict[str, Dict[str, Union[ShapeDict, TableDict]]]  # JSON-serializable inventory
...
class TableData:
    """Data structure for table properties extracted from a PowerPoint table."""

    def __init__(self, shape: BaseShape, slide: Optional[Any] = None):
        """Initialize from a PowerPoint table shape.

        Args:
            shape: The PowerPoint table shape
            slide: Optional slide object for context
        """
        self.shape = shape
        self.table_id: str = ""  # Will be set after sorting

        # Get position and dimensions
        self.left: float = round(ShapeData.emu_to_inches(shape.left), 2) if hasattr(shape, "left") else 0
        self.top: float = round(ShapeData.emu_to_inches(shape.top), 2) if hasattr(shape, "top") else 0
        self.width: float = round(ShapeData.emu_to_inches(shape.width), 2) if hasattr(shape, "width") else 0
        self.height: float = round(ShapeData.emu_to_inches(shape.height), 2) if hasattr(shape, "height") else 0

        # Extract table structure
        table = shape.table  # type: ignore
        self.rows: int = len(table.rows)
        self.columns: int = len(table.columns)

        # Extract cell content - cells indexed by "row,col"
        ...
        for row_idx, row in enumerate(table.rows):
            for col_idx, cell in enumerate(row.cells):
                ...
                if hasattr(cell, "is_merge_origin") and cell.is_merge_origin:
                    self.cells[cell_key]["is_merge_origin"] = True
                if hasattr(cell, "is_spanned") and cell.is_spanned:
                    self.cells[cell_key]["is_spanned"] = True

    @property
    def has_any_issues(self) -> bool:
        """Tables don't have overflow issues tracked."""
        return False
...
def extract_text_inventory(
    ...
    for slide_idx, slide in enumerate(prs.slides):
        slide_inventory: Dict[str, Union[ShapeData, TableData]] = {}
        ...
        if shapes_with_positions:
            ...
            # Add shapes to slide inventory
            for shape_data in sorted_shapes:
                slide_inventory[shape_data.shape_id] = shape_data

        # Process tables
        if tables:
            # Sort tables by visual position
            tables = sorted(tables, key=lambda t: (t.top, t.left))
            for idx, table_data in enumerate(tables):
                table_data.table_id = f"table-{idx}"
                slide_inventory[table_data.table_id] = table_data

        if not slide_inventory:
            continue

        # Create slide inventory
        inventory[f"slide-{slide_idx}"] = slide_inventory
...
def get_inventory_as_dict(pptx_path: Path, issues_only: bool = False) -> InventoryDict:
    ...
    dictionaries instead of ShapeData/TableData objects, useful for testing and direct
    JSON serialization.
    ...
    # Convert ShapeData/TableData objects to dictionaries
    dict_inventory: InventoryDict = {}
    for slide_key, items in inventory.items():
        dict_inventory[slide_key] = {
            item_key: item_data.to_dict() for item_key, item_data in items.items()
        }
