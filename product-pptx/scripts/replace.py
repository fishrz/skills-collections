...
Shapes can be deleted entirely by setting "delete": true in the replacement JSON.
This is useful for removing guideline/instruction boxes from templates.
"""
...
from inventory import InventoryData, extract_text_inventory, TableData
...
def apply_table_replacements(table_shape, replacement_data: Dict[str, Any]) -> int:
    """Apply cell replacements to a table.

    Args:
        table_shape: The table shape from python-pptx
        replacement_data: Dict with 'cells' key containing cell replacements

    Returns:
        Number of cells replaced
    """
    cells_replaced = 0
    cells_data = replacement_data.get("cells", {})
    ...
    for cell_key, cell_data in cells_data.items():
        # Parse cell key "row,col"
        try:
            row_idx, col_idx = map(int, cell_key.split(","))
        except ValueError:
            print(f"  WARNING: Invalid cell key format '{cell_key}', expected 'row,col'")
            continue

        # Check bounds
        if row_idx >= len(table.rows) or col_idx >= len(table.columns):
            print(f"  WARNING: Cell {cell_key} out of bounds (table is {len(table.rows)}x{len(table.columns)})")
            continue
        ...
        # Preserve first paragraph for formatting, clear runs
        if cell.text_frame.paragraphs:
            ...
            if para.runs:
                ...
                apply_font_properties(run, cell_data)

            # Clear additional paragraphs if any
...
def detect_frame_overflow(inventory: InventoryData) -> Dict[str, Dict[str, float]]:
    ...
    for slide_key, shapes_dict in inventory.items():
        for shape_key, shape_data in shapes_dict.items():
            # Skip tables - they don't have frame_overflow_bottom
            if isinstance(shape_data, TableData):
                continue
...
def validate_replacements(inventory: InventoryData, replacements: Dict) -> List[str]:
    """Validate that all shapes/tables in replacements exist in inventory.

    Returns list of error messages.
    """
    errors = []

    for slide_key, items_data in replacements.items():
        ...
        # Check each shape/table
        for item_key in items_data.keys():
            if item_key not in inventory[slide_key]:
                # Find items without replacements defined and show their content
                unused_with_content = []
                for k in inventory[slide_key].keys():
                    if k not in items_data:
                        item_data = inventory[slide_key][k]
                        # Get text preview based on type
                        if hasattr(item_data, 'paragraphs'):
                            paragraphs = item_data.paragraphs
                        ...
                        elif hasattr(item_data, 'cells'):
                            # Table - show dimensions
                            unused_with_content.append(f"{k} (table {item_data.rows}x{item_data.columns})")
                        else:
                            unused_with_content.append(k)

                errors.append(
                    f"Item '{item_key}' not found on '{slide_key}'. "
                    f"Items without replacements: {', '.join(sorted(unused_with_content)) if unused_with_content else 'none'}"
                )
...
def apply_replacements(pptx_file: str, json_file: str, output_file: str):
    ...
    # Load replacement data with duplicate key detection and UTF-8 encoding
    ...
    shapes_deleted = 0
    tables_processed = 0
    table_cells_replaced = 0

    # Collect shapes to delete (must delete after processing to avoid index issues)
    shapes_to_delete = []  # List of (slide, shape) tuples

    # Process each slide from inventory
    for slide_key, items_dict in inventory.items():
        if not slide_key.startswith("slide-"):
            continue
        ...
        # Process each item (shape or table) from inventory
        for item_key, item_data in items_dict.items():
            # Check if this is a table
            if isinstance(item_data, TableData):
                tables_processed += 1
                replacement_data = replacements.get(slide_key, {}).get(item_key, {})
                if "cells" in replacement_data:
                    cells_count = apply_table_replacements(item_data.shape, replacement_data)
                    table_cells_replaced += cells_count
                continue
            ...
            if not shape:
                print(f"Warning: {item_key} has no shape reference")
                continue

            # Check for replacement data
            replacement_shape_data = replacements.get(slide_key, {}).get(item_key, {})

            # Check if shape should be deleted
            if replacement_shape_data.get("delete", False):
                slide = prs.slides[slide_index]
                shapes_to_delete.append((slide, shape))
                shapes_deleted += 1
                continue
    ...
    # Delete shapes marked for deletion
    for slide, shape in shapes_to_delete:
        sp = shape._element
    ...
    # Collect warnings from updated shapes (tables don't have warnings)
    warnings = []
    for slide_key, items_dict in updated_inventory.items():
        for item_key, item_data in items_dict.items():
            if hasattr(item_data, 'warnings') and item_data.warnings:
                for warning in item_data.warnings:
                    warnings.append(f"{slide_key}/{item_key}: {warning}")
    ...
    print(f"  - Shapes deleted: {shapes_deleted}")
    if tables_processed > 0:
        print(f"  - Tables processed: {tables_processed}")
        print(f"  - Table cells replaced: {table_cells_replaced}")
